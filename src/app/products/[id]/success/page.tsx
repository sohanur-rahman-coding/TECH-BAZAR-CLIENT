  ...
  if (loading) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-slate-400">
        ...
        <h2 className="text-xl font-bold tracking-tight">Securing Your Gadget...</h2>
        <p className="text-sm mt-2">Please wait while we finalize the escrow transaction.</p>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-slate-400">
        ...
        <h2 className="text-2xl font-black text-slate-100 uppercase">Purchase Error</h2>
        <p className="text-sm mt-3 max-w-md text-center">{error}</p>
        <Link href={`/products/${id}`} className="mt-8 px-6 py-3 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-xl transition">
          Return to Product
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-[70vh] flex flex-col items-center justify-center text-center max-w-2xl mx-auto px-4 py-20">
      ...
      <div className="relative mb-8">
        ...
        <div className="relative h-24 w-24 bg-gradient-to-tr from-violet-600 to-emerald-500 rounded-full flex items-center justify-center shadow-2xl border border-white/10">
      ...
      <div className="flex flex-col sm:flex-row items-center gap-4 w-full sm:w-auto">
        <Link href="/products" className="w-full sm:w-auto px-8 py-4 bg-violet-600 hover:bg-violet-500 text-white font-black rounded-xl transition shadow-[0_0_20px_rgba(139,92,246,0.3)] hover:shadow-[0_0_30px_rgba(139,92,246,0.5)] flex items-center justify-center gap-2">
      ...
      <div className="mt-12 bg-slate-900/50 border border-slate-800 p-6 rounded-2xl max-w-md w-full">
        <p className="text-xs font-semibold text-slate-500 uppercase tracking-widest mb-3">Order Details</p>
        <div className="flex justify-between items-center text-sm">
          <span className="text-slate-400">Order ID:</span>
          <span className="font-mono text-slate-200">{sessionId?.substring(0, 16)}...</span>
        </div>
        ...
        <div className="flex justify-between items-center text-sm mt-2">
          ...
          <span className="text-emerald-400 font-bold">Secured in Escrow</span>
        </div>
      </div>
    </div>
  );
